...
Use `addTool(definition)` for new tools. Use `updateTool(definition)` to replace an existing tool with a complete definition. Use `removeTool(name)` to remove an existing tool. The `code` value must be a self-contained async function string that runs in page context. Verify the added tool by calling `await tool.execute(input)` with representative input before finishing the task.
